...
export function Login() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      console.log("[v0] Attempting Supabase login with:", credentials.id)

      // Convert username to email format for Supabase
      const email = credentials.id.includes("@") ? credentials.id : `${credentials.id}@sitrack.gov.id`
      ...
      console.log("[v0] Supabase auth result:", { user: authData.user?.email, error: authError })
      ...
      if (authData.user) {
        ...
        console.log("[v0] Logging in user:", user)
