...
import { getWorkflowActionDetails } from "../../../lib/workflow-history"
...
export function WorkflowActions({ report, userRole, onWorkflowAction }: WorkflowActionsProps) {
  ...
  const handleSubmitAction = async () => {
    try {
      const actionDetails = getWorkflowActionDetails(selectedAction)
      const finalNotes = notes || actionDetails.notes

      console.log("[v0] Submitting workflow action:", {
        reportId: report.id,
      ...
      onWorkflowAction(report.id, selectedAction, finalNotes)

      setShowNotesModal(false)
      setSelectedAction("")
      setNotes("")
    } catch (error) {
      console.error("[v0] Error in workflow action:", error)
