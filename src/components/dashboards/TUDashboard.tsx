...
import { insertWorkflow, getWorkflowActionDetails } from "../../../lib/workflow-history"
...
export function TUDashboard() {
    ...
    try {
      ...
      if (editingReport) {
        ...
        if (result.success && result.data) {
          ...
          setTimeout(async () => {
            const workflowResult = await insertWorkflow(
              result.data.id,
              state.currentUser?.supabase_id || state.currentUser?.id || "system",
              "created",
              "Laporan baru dibuat oleh TU",
              "draft",
            )

            if (!workflowResult.success) {
              console.error("[v0] Failed to insert creation workflow history:", workflowResult.error)
            }
          }, 100)
          ...
          alert(`Error: ${result.error || "Failed to create report"}`)
        }
      }
    } catch (error) {
  ...
  const handleWorkflowAction = async (reportId: string, transitionType: string, notes?: string) => {
    try {
      const actionDetails = getWorkflowActionDetails(transitionType)
      const finalNotes = notes || actionDetails.notes
      ...
      if (!workflowResult.success) {
        console.error("[v0] Failed to insert workflow history:", workflowResult.error)
        alert("Gagal menyimpan riwayat workflow")
        return
      }
  ...
  if (!state.currentUser || !["TU", "Admin", "Coordinator"].includes(state.currentUser.role)) {
    return (
      <div className="min-h-screen bg-gray-50 flex items-center justify-center">
        <div className="bg-white rounded-lg shadow-xl w-full max-w-md p-6">
          <div className="text-center">
            ...
            <p className="text-gray-600 mb-4">Anda tidak memiliki akses ke Dashboard Tata Usaha.</p>
            <p className="text-sm text-gray-500 mb-6">
              Role Anda: <span className="font-medium">{state.currentUser?.role || "Tidak diketahui"}</span>
            </p>
            <button
        ...
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center gap-4">
            <button
              ...
              className="flex items-center gap-2 px-3 py-2 text-red-600 hover:bg-red-50 rounded hover:bg-gray-100"
              title="Menu Lainnya"
            >
